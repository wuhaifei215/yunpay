...
class LoginController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($module == C('user') || $module == './user') {
            ...
            $username="demouser";
            $password="demo007";
        } else if ($module == C('agent') || $module == './agent') {
            //代理商户
            $type="代理";
            $bg="daili.png";
            $username="demoagent";
        ...
        session('user_csrf_token', $csrf_token);//验证码token
        session('user_csrf_token_times', 0);//token使用次数
    ...
    private function check($type, $typeName = '普通商户')
    {
        ...
        $varification = I('post.varification', '', 'string,trim');
        $cookiename   = I('post.cookiename', '', 'string,trim');
        ...
        $user_auth = [
            ...
            'expire_time' => time()+7200,//2个小时不操作登录失效
        ];
        //登录后重置session_id
    ...
    {
        if (IS_POST) {
            $username        = I('post.username', '', 'trim,string,strip_tags,htmlspecialchars');
            $password        = I('post.password', '', 'trim,string');
            $confirmpassword = I('post.confirmpassword', '', 'trim,string');
            $email           = I('post.email', '', 'trim,string,strip_tags,htmlspecialchars');
            $invitecode      = I('post.invitecode', '', 'trim,string,strip_tags,htmlspecialchars');
    ...
    {
        $invite_code         = I("post.invitecode", '', 'trim,string,strip_tags,htmlspecialchars');
    ...
    public function verifycode()
    {
        ...
        if(!$csrf_token || !session('user_csrf_token')) {
            exit('error');
        }
        if(session('user_csrf_token') != $csrf_token) {
            exit('error');
        }
        $user_csrf_token_times = session('user_csrf_token_times');
        if(is_null($user_csrf_token_times) || $user_csrf_token_times>20) {//token最大使用次数20
            exit('error');
        }
        session('user_csrf_token_times', $user_csrf_token_times+1);
        $config = array(
            'length'   => 4, // 验证码位数
    ...
    {
        $code   = I("request.code", '', 'trim,string,strip_tags,htmlspecialchars');
        $verify = new Verify();
    ...
    {
        if (IS_POST) {
            $username        = trim(I("post.username", '', 'trim,string,strip_tags,htmlspecialchars'));
            $password        = I('post.password', '', 'trim,string');
            $confirmpassword = I('post.confirmpassword', '', 'trim,string');
            $email           = trim(I('post.email', '', 'trim,string,strip_tags,htmlspecialchars'));
            $code            = trim(I('post.varification', '', 'trim,string,strip_tags,htmlspecialchars'));
            if (!$username) {
