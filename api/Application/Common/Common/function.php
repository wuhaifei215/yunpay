...
function huoquddlx($transid)
{
    ...
    $ddlx = $Order->table($Order->getRealTableName($date))->where(["pay_orderid" => $transid])->getField("ddlx");
 ...
 * @return boolean
 */
function checkNotifyurlIp($ip, $ip_recorded) {
    if(!$ip) {
        $ip = get_client_ip();
    }
    $ip_arr = explode(",", $ip_recorded);
    if(empty($ip_arr)) {
        return false;
    }
...
function logApiAdd($title, $method, $memberid, $orderid, $business_type, $oper_param=[], $ip){
    $log = [
        'title' => $title,        //主题
        'method' => $method,      //程序方法名称
        'memberid' => $memberid,
        'order_id' => $orderid,   //订单号
        'business_type' => $business_type,    //业务类型（1代收 2代付）
        'oper_param' => $oper_param,      //请求参数
        'oper_ip' => $ip,
        'create_time' => date('Y-m-d H:i:s'),       //创建时间
    ];
    $url = C('LOG_API_URL') . '/Log_Api_addLog.html';
    // log_place_order('logApiaddLog', "----提交地址", $url);    //日志
    // log_place_order('logApiaddLog', "----提交", json_encode($log, JSON_UNESCAPED_UNICODE));    //日志
    $res = http_post_json($url, $log);
    // log_place_order('logApiaddLog', "----返回", json_encode($res, JSON_UNESCAPED_UNICODE));    //日志
 ...
 * @param $orderid  订单号
 */
function check_IP($channelid, $ip, $orderid)
{
    $syschannel = M('Channel')->where(['id' => intval($channelid)])->find();
    $notifyips = explode(',', $syschannel['notifyip']);
    if (trim($notifyips[0]) != '' && !in_array($ip, $notifyips)) {
        log_place_order($syschannel['code'] . '_attack', $orderid . "----异常IP", $ip);
        return false;
    } else {
        return true;
    }
    ...
    if (is_string($value)) {
        foreach ($arr as $a) {
            ...
            if (stripos($value, $a) !== false) {
                log_place_order('_attack_data', "参数错误，含有敏感字符", json_encode($value));
                return false;
            }
        }
 ...
 * @return bool
 */
function Reject($Reject,$return=array(), $tableName='',$notifyurl=1) {
    $id = $Reject['id'];
...
// function Automatic_Notify ($id, $tableName='') {
    ...
    //     'orderid' => $wttklist["out_trade_no"], //商户订单号
    //     "transaction_id" => $wttklist['orderid'], //平台订单号
