...
class CreateDFController extends Controller
{
    ...
    public function payoutVN(){
        ...
        if (!$bankname) {
            ...
            //越南系统银行
            $check_bank = M('Systembank')->where(['bankcode' => $bankname, 'currency'=> 'VND'])->count();
            // //判断是否有该银行编码
    ...
    //巴基斯坦代付接口
    public function payoutBD(){
        $this->channelIsOpen(); //判断通道是否开启
        
        $bankcode = I('request.bankcode', 0, 'intval');
        if (!in_array($bankcode,[924 ,925, 926])) {
            $this->showmessage('bankcode error');
        }
        $bankname = I("request.bankname", '', 'string,strip_tags,htmlspecialchars');
        if (!$bankname) {
    ...
    protected function doNext()
    {
        ...
        $postData['get_time'] = microtime(TRUE);
        $user_id = $this->memberid - 10000;
        // logApiAdd('商户代付提交YunPay', __METHOD__, $user_id, $out_trade_no, 2, $postData, getRealIp());
    
        if($user_id === 2 || $user_id === 25) {
        ...
        if($this->merchants['country_id'] > 1){       //排除菲律宾
            $currency = I('request.currency', '', 'string,strip_tags,htmlspecialchars');
            if(!$currency){
                $this->showmessage("currency error");
            }
            $check_country = D('Country')->where(['id' => $this->merchants['country_id'], 'currency' =>$currency])->find();
            //用户国家信息
            if(!$check_country){
                $this->showmessage("The merchant's currency error");
            }
        }
