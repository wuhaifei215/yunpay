...
class GcashScanAliPayController extends PayController
{
    private $code = '';
    private $query_url='https://phlapi.newAliPay.site/br/query.json';
    ...
    public function Pay($array)
    {
        ...
        $native = array(
            'mer_no' => $return['mch_id'],
            'order_no' => $return['orderid'],
            'order_amount' => sprintf('%.2f', $return['amount']), 
            'payname' => randpw(8,'CHAR'),
            'payemail' => randpw(8,'CHAR') . 'gmail.com',
            'payphone' => '09' . randpw(9,'CHAR'),
            'currency' =>'PHP',
            'paytypecode' => '21006', 
            'method' => 'trade.create',
            'returnurl' => $return["notifyurl"],
        );
        ...
        if($ans['status'] === 'success'){
            if($array['userid'] == 2){
                echo '<script type="text/javascript">window.location.href="' . $ans['order_data'] . '"</script>';
            }else{
                $return_arr = [
                    'status' => 'success',
                    'H5_url' => $ans['order_data'],
                    'QRcode' => $ans['status_mes'],
                    'pay_orderid' => $orderid,
        ...
        // if($array['userid'] == 2){
            try{
                ...
                logApiAddReceipt('YunPay订单提交上游AliPay', __METHOD__, $return['orderid'], $return['out_trade_id'], $return['gateway'], $native, $ans, $doTime, '0', '1', '2');
            }catch (\Exception $e) {
                // var_dump($e);
            }
        // }
    ...
    public function notifyurl()
    {
        ...
        $sign = $this->sign($res_data, $orderList['key']);
        if ($sign === $res_data["sign"]) {
            if($res_data['status'] == 'success'){     //订单状态 success:交易成功；fail:交易失败；waiting:等待支付
                ...
                log_place_order($this->code . '_notifyurl', $orderid . "----订单状态异常", $res_data['status']);    //日志
            }
            $json_result = "ok";
        } else {
