...
class H88PayGcashController extends PayController
{
    private $code = '';
    private $query_url='https://pay.h88pay.com/pay/order/trade';
    ...
    public function Pay($array)
    {
        ...
        $native = array(
            ...
            'return_type' => "json",        //返回类型，默认为表单提交，传值json,接口返回pay_url,传info,系统将返回支付信息，商户需自行处理(如需使用info请联系平台管理员)  
        );
        ...
        // if($array['userid'] == 2){
            try{
                ...
                logApiAddReceipt('YunPay订单提交上游H88pay', __METHOD__, $return['orderid'], $return['out_trade_id'], $return['gateway'], $native, $ans, $doTime, '0', '1', '2');
            }catch (\Exception $e) {
                // var_dump($e);
            }
        // }
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if($result['state'] == 1){     //订单状态 1支付成功，其他失败
                ...
                log_place_order($this->code . '_notifyurl', $orderid . "----订单状态异常", $result['state']);    //日志
            }
            $json_result = "success";
        } else {
    ...
    //     $returnContent = $this->http_post_data($this->query_url, $native);
    //     log_place_order($this->code. '_queryOrder', $orderid . "----查单返回", $returnContent);    //日志
    //     $ans = json_decode($returnContent, true);
    //     if ($ans['code'] === 1 && $ans['data']['state']===1) {      //1为成功，0待支付，其余为失败  
    //         return 1;
    //     } else {
