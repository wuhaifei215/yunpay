...
function encryptDecrypt($string, $key = '', $decrypt = '0')
{
    if ($decrypt) {
        ...
        // return $decrypted;
        $data = openssl_decrypt(base64_decode($string), 'DES-ECB', $key, OPENSSL_RAW_DATA);
        return $data;
    } else {
        // $encrypted = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $string, MCRYPT_MODE_CBC, md5(md5($key))));
        // return $encrypted;
        $data = openssl_encrypt($string, 'DES-ECB', $key, OPENSSL_RAW_DATA);
...
function getKey($orderid)
{
    ...
    $key = $Order->table($Order->getRealTableName($date))->where(['pay_orderid' => $orderid])->getField('key');
    return $key;
}
