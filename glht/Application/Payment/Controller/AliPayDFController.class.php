...
class AliPayDFController extends PaymentController
{
    private $code = '';
    ...
    public function PaymentExec($data, $config)
    {
        ...
        $post_data = array(
            "mer_no" => $config['mch_id'], //商户号
            "order_no" => $data['orderid'], //订单号
            'method' =>'fund.apply',
            "order_amount" => sprintf('%.2f', $data['money']), 
            "currency" =>'PHP',
            "acc_code" => $cardType,
            "acc_name" => $data['bankfullname'],  //户名
            "acc_no" => $data['banknumber'],    //账号
            "acc_email" => '123456789@gmail.com',
            "acc_phone" => '12345678901',
            "returnurl" => 'https://' . C('NOTIFY_DOMAIN') . "/Payment_" . $this->code . "_notifyurl.html",      //异步通知地址
        );
        ...
        if($result['status'] === 'success'){
            ...
            $return = ['status' => 0, 'msg' => $result['status_mes']];
        }
        return $return;
    }
    ...
    public function notifyurl()
    {
        ...
        if ($sign === $re_data["sign"]) {
            if ($re_data['result'] === "success") {     //订单状态 success:交易成功；fail:交易失败；waiting:等待支付
                //代付成功 更改代付状态 完善代付逻辑
                $data = [
                    'memo' => '代付成功',
                ];
                ...
                $json_result = "ok";
            } elseif ($re_data['result'] === "fail") {
                //代付失败
                $data = [
                    'memo' => '代付失败-' . $re_data['ext_info'],
                ];
    ...
    public function queryBalance()
    {
        if (IS_AJAX) {
            ...
            if($result['checkstatus'] === 'success'){
                $available = $result['currency']['PHP'];  //可用金额
                $html = <<<AAA
...
AAA;
    ...
    public function PaymentQuery($data, $config)
    {
        ...
        $result = json_decode($returnContent, true);
        if ($result['checkstatus'] === "success") {
            switch ($result['resultstatus']) {       //success:出款成功；fail:出款失败；waiting:处理中
                case 'waiting':
