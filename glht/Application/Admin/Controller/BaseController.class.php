...
class BaseController extends Controller{
    ...
    public function __construct()
    {
        ...
        if(session('admin_auth.expire_time')<= time()) {
            ...
            // if(!(CONTROLLER_NAME == 'Withdrawal' && ACTION_NAME == 'checkNotice')){
                ...
                $admin_auth = [
                    ...
                    'groupid' => $admin_auth1['groupid'],
                    'password' => $admin_auth1['password'],
                    'session_random' => $admin_auth1['session_random'],
                    'expire_time' => time() + 7200,//2个小时不操作登录失效
                ];
                session('admin_auth', $admin_auth);
                ksort($admin_auth);
                $code = http_build_query($admin_auth);
                $sign = sha1($code);
        ...
        if(CONTROLLER_NAME != 'Login' && !IS_ROOT&&$name!="System/editPassword" && $name!="Auth/google" && $name!="Index/index" && $name!="Index/MP3File"){
            $auth = new Auth();
