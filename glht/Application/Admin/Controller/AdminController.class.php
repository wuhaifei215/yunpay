...
class AdminController extends BaseController
{
    ...
    public function addAdmin(){
        if(IS_POST){
            UserLogService::HTwrite(2, '添加管理员', '添加管理员');
            //防止跨站请求伪造
    ...
    {
        UserLogService::HTwrite(4, '删除管理员', '删除管理员');
        if (parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) !== C('DOMAIN')) {
            $this->ajaxReturn(['status' => 0, 'msg' => 'Denied']);
        }
        $id = I('id', 0, 'intval');
        ...
        if ($change_result) {
            UserLogService::HTwrite(4, '删除管理员', '删除（' . $id . '）管理员');
            M("auth_group_access")->where(array("uid" => $id))->delete();
            UserLogService::HTwrite(4, '删除管理员权限', '删除（' . $id . '）管理员权限');
        }
    ...
    public function editAdmin(){
        if(IS_POST){
            UserLogService::HTwrite(3, '编辑管理员', '编辑管理员');
            if (parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) !== C('DOMAIN')) {
                $this->ajaxReturn(['status' => 0, 'msg' => 'Denied']);
            }
            $data=I("post.");
            ...
            if($admin_result!==false){
                UserLogService::HTwrite(3, '编辑管理员成功', '编辑管理员（' . $data["username"] . '）成功');
                //更新权限
                $groupAccess= M("auth_group_access")->where(array("uid"=>$data['id']))->find();

                if ($groupAccess) {
                    UserLogService::HTwrite(3, '编辑管理员权限成功', '编辑管理员（' . $data["username"] . '）权限成功');
                    M("auth_group_access")->where(array("uid" => $data['id'], "group_id" => $groupAccess["group_id"]))->setField("group_id", $data["groupid"]);
                } else {
                    UserLogService::HTwrite(2, '添加管理员权限成功', '加管理员（' . $data["username"] . '）权限成功');
                    M("auth_group_access")->add(array("uid" => $data['id'], "group_id" => $data["groupid"]));
                }
                $this->ajaxReturn(['status' => 1, 'msg' => '修改成功!']);
            } else {
                UserLogService::HTwrite(2, '添加管理员失败', '添加理员（' . $data["username"] . '）失败');
