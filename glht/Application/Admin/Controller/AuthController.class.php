...
class AuthController extends BaseController
{
    ...
    {
        if(IS_POST){
            UserLogService::HTwrite(2, '添加管理员角色', '添加管理员角色');
            ...
            if($add_group_result){
                UserLogService::HTwrite(2, '添加管理员角色成功', '添加管理员角色成功');
            }
    ...
    {
        if(IS_POST){
            UserLogService::HTwrite(3, '编辑管理员角色', '编辑管理员角色');
            $params = array(
                'id' => I('id', 0, 'intval'),
                'title' => I('title'),
            );
            ...
            if($save_group_result){
                UserLogService::HTwrite(3, '管理员角色修改成功', '管理员角色修改成功');
            }
    ...
    {
        UserLogService::HTwrite(4, '删除管理员角色', '删除管理员角色');
        $id = I('id', 0, 'intval');
        if(!$id){
    ...
    {
        if(IS_POST){
            UserLogService::HTwrite(3, '分配管理员角色', '分配管理员角色');
            ...
            if(!empty($group_id)){
                ...
                UserLogService::HTwrite(4, '删除管理员原有角色', '删除管理员id（'.$user_id.'）原有角色');
                foreach($group_id as $v){
                    ...
                    UserLogService::HTwrite(2, '重新添加角色', '重新添加管理员id（'.$user_id.'）角色'.json_encode($add_data));
                }
            }
    ...
    public function ruleGroup()
    {
        ...
        $admin_auth_group_model = D('AdminAuthGroup');
        if(IS_POST){
            UserLogService::HTwrite(3, '分配角色权限', '分配角色权限');
            $data = I('post.');
            ...
            if($admin_auth_group_model->addAuthRule($rule_ids, $role_id) !== false){
                UserLogService::HTwrite(3, '分配角色权限成功', '分配角色权限成功');
                $this->ajaxReturn(['status'=>1,'msg'=>'分配成功']);
            }else{
                UserLogService::HTwrite(3, '分配角色权限失败', '分配角色权限失败');
    ...
    public function google()
    {
        ...
        if (!session('admin_auth')) {
            $this->error('未登录','/' . C("HOUTAINAME"));
        }
        $google_auth = session('google_auth');
        if($google_auth) {
            $this->redirect('/' . C("HOUTAINAME"));
        }
            ...
            if($action_type == 0) {//首次绑定
                ...
                if(false === $ga->verifyCode($google_secret_key, $code, C('google_discrepancy'))) {
                    ...
                    if(FALSE !== $re) {
                        ...
                        $this->success("绑定成功",'/' . C("HOUTAINAME"));
                    } else {
                ...
                if(false === $ga->verifyCode($google_secret_key, $code, C('google_discrepancy'))) {
                    ...
                    clear_auth_error($uid,5);
                    session('google_auth', $code);
                    $this->success("验证通过，正在进入系统...", '/' . C("HOUTAINAME"));
                }
            }
    ...
    public function unbindGoogle()
    {
        $uid = session('admin_auth')['uid'];
        if(IS_POST) {
            UserLogService::HTwrite(3, '解绑谷歌身份验证', '解绑谷歌身份验证');
            //验证短信验证码
            $code        = I('request.code');
            ...
            if(FALSE !== $re) {
                ...
                UserLogService::HTwrite(3, '解绑谷歌身份验证成功', '解绑谷歌身份验证成功');
                $this->ajaxReturn(["status"=>1, "msg"=>"解绑成功"]);
            } else {
                UserLogService::HTwrite(3, '解绑谷歌身份验证失败', '解绑谷歌身份验证失败');
    ...
    {
        UserLogService::HTwrite(3, '解绑谷歌身份验证器验证码', '解绑谷歌身份验证器验证码');
        $uid = session('admin_auth')['uid'];
