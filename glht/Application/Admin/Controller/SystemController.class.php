...
class SystemController extends BaseController
{
    ...
    {
        if (IS_POST) {
            UserLogService::HTwrite(3, '修改管理员密码', '修改管理员密码');
            ...
            UserLogService::HTwrite(3, '修改管理员密码成功', '修改管理员密码成功');
    ...
    {
        UserLogService::HTwrite(3, '更改基本设置', '更改基本设置');
        if (IS_POST) {
            ...
            if ($id) {
                ...
                UserLogService::HTwrite(2, '添加基本设置', '添加基本设置');
            }
            if (FALSE === $res) {
                UserLogService::HTwrite(3, '更改基本设置失败', '更改基本设置失败');
                ...
                UserLogService::HTwrite(3, '更改基本设置成功', '更改基本设置成功'.json_encode($res));
...
EOD;
    ...
    public function uploadImg()
    {
        if (IS_POST) {
            ...
            if (!$info) {
                ...
                if($res) {
                    $this->ajaxReturn(['status' => $res, 'msg' => '上传成功', 'data' => $data['logo']]);
                } else {
    ...
    public function clearData() {
        ...
        $verifyGoogle = adminGoogleBind($uid);
        if (IS_POST) {
            UserLogService::HTwrite(4, '数据清理', '数据清理');
            $data = I('post.');
            $type_ids = $data['type'];
            if(empty($type_ids)){
            ...
            foreach($type_ids as $v) {
                if($v == 1) { //入金记录
                    ...
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '入金记录清理失败', '入金记录清理失败');
                    ...
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, 'wttklist代付提交记录清理失败', 'wttklist代付提交记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                    $res = M('tklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, 'tklist代付记录清理失败', 'tklist代付记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                    $res = M('df_api_order')->where(['create_time'=>['between', [$startTime, $endTime]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, 'df_api_order代付API记录清理失败', 'df_api_order代付API记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 3) {//登录记录
                    $res = M('loginrecord')->where(['logindatetime'=>['between',[date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '登录记录清理失败', '登录记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 4) {//冻结记录
                    ...
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '资金冻结待解冻记录清理失败', '资金冻结待解冻记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                    $res = M('complaints_deposit')->where(['create_at'=>['between', [$startTime, $endTime]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '投诉保证金余额清理失败', '投诉保证金余额清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 5) {//资金记录
                    ...
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '资金记录清理失败', '资金记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                }elseif($v == 6) {//手动结算记录
                    $res = M('tklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '手动结算记录清理失败', '手动结算记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                }elseif($v == 7) {//操作记录
                    $res = M('user_log')->where(['create_at'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        UserLogService::HTwrite(4, '操作记录清理失败', '操作记录清理失败');
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                }
            }
            M()->commit();
            UserLogService::HTwrite(4, '数据清理成功', '数据清理成功');
            $this->ajaxReturn(['status' => 1, 'msg' => "清理成功！"]);
        } else {
            UserLogService::HTwrite(1, '数据清理', '数据清理');
            $uid = session('admin_auth')['uid'];
            $user = M('Admin')->where(['id'=>$uid])->find();
            $this->assign('mobile', $user['mobile']);
            $this->assign('verifysms', $verifysms);
